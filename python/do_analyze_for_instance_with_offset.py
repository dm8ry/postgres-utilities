...
for row in rows:
    ...
    idx2=1
    for row2 in rows2:
       cur2.execute("analyze", row2[0])
       print("[db:{0}|port:{1}] ({3}|{4}) analyze {2}".format(inp_inp_database, inp_port, row2[0], idx2, len(rows2)))
       idx2=idx2+1

    nTables2=len(rows2)

    print(" ")
    print("* Total: {0} tables analyzed in Postgres DB: {1}, Port: {2}, Host: {3}".format(nTables2, inp_inp_user, inp_port, inp_hostname))
    print(" ")
